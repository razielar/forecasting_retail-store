...
\begin{enumerate}
\item \textit{initial}: the size of the initial training period. Default: 3*horizon.
\item \textit{period}: space between two or more training periods, \textit{i.e.} cutoff dates. Default: 0.5*horizon.
\item \textit{horizon}:  forecast horizon, \textit{i.e.} how many days/weeks,months,years are we going to make a forecast on. 
\end{enumerate}  

In our work, since we have 1087 days we are using the initial 730 days (67.16\%) to predict the next 365 days, although all the results and plots presented show the performance metric (see \nameref{sec:error}) of a half year horizon. The period used was 180 days. \autoref{tab:cv} presents a more detailed description of the used parameters in the cross validation function.
...
\begin{table}[!htb]
  ...
  \begin{scriptsize}
    \begin{tabulary}{0.65\linewidth}{ccc}
      ...
      \textit{period} &  0.5*horizon: 182 days & 180 days   \\
